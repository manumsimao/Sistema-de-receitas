...
const adapterReceitas = new FileSync("./public/database/receitas.json");
const dbReceitas = low(adapterReceitas);
...
app.post("/autenticar", function (req, res) {
	...
	if (dbUsuarios.has(usuario) == true) {
		if (dbUsuarios.getState()[usuario]["senha"] == senha) {
			//Sucesso
...
app.post("/api/receitas", async function (req, res) {
	...
	const response = await dbReceitas
		.push({
			id: id,
			nome: nome,
			secao: secao,
...
app.put("/receitas/:nomeReceita", function (req, res) {
	...
	if (USUARIO_PERMISSAO_ATUAL == "") {
		...
		res.render("infoReceita", {
			nomeUsuario: USUARIO_NOME_ATUAL,
			permissaoUsuario: USUARIO_PERMISSAO_ATUAL,
			titulo: titulo,
			acao: "edit",
		});
	}
...
app.delete("/receitas/:nomeReceita", function (req, res) {
	nomeReceita = req.params.nomeReceita;
  dbReceitas.remove({id:nomeReceita}).write();
});
